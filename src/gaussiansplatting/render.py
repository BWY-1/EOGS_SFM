...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, scene_params, resolution):
    ...
    if 'IARPA' in base_path:
        ...
        print(f"[WARN] Unknown dataset name in path '{base_path}', using --res={resolution}")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        xoff = np.floor(xmin / dataset_resolution) * dataset_resolution
        xsize = int(1 + np.floor((xmax - xoff) / dataset_resolution))
        yoff = np.ceil(ymax / dataset_resolution) * dataset_resolution
        ysize = int(1 - np.floor((ymin - yoff) / dataset_resolution))

        # run plyflatten
        # dsm = plyflatten(cloud, xoff, yoff, resolution, xsize, ysize, radius=1, sigma=float("inf"))
        dsm = plyflatten(cloud, xoff, yoff, dataset_resolution, xsize, ysize, radius=1, sigma=float("inf"))
